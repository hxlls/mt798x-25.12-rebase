	...
	if (short_pkt_toggle && !(skb->mark & MTK_QDMA_QUEUE_MASK)) {
		if (h_proto == ETH_P_IP) {
			...
			if (iph->protocol == IPPROTO_TCP) {
				...
				if (payload_len == 0) {
					qid = short_pkt_qid;
					pr_info("hnat shortpkt: TCP ACK -> qid %d\n", qid);
				}
				...
				if (payload_len <= short_pkt_len) {
					qid = short_pkt_qid;
					pr_info("hnat shortpkt: UDP len=%u -> qid %d\n",
						payload_len, qid);
				}
			}
